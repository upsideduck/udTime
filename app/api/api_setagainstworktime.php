<?php 
/************************************************************************
/*
/*	Adds free days
/*
/*
/*	Requires: 			
/*	
/*	Post values:	
/*
/*	Output:
/*
/************************************************************************/

$result_arr = null;

$starttime = clean($_REQUEST['starttime']);
$endtime = clean($_REQUEST['endtime']);
$type = clean($_REQUEST['type']);
if (strlen($_REQUEST['time']) == 8 && strpos($_REQUEST['time'],":") != false) $time = strtotime($_REQUEST['time']);
else $time = intval(clean($_REQUEST['time']));
if($time == 0 || !is_integer($time)) $time = 0;

$timespan = new timespan($starttime, $endtime);
$result_free = $timespan->setagainstworktime($time,$type);

if($result_free){
	$i = 0;
	foreach($result_free as $result){
		$result_arr[0] = true;
		$result_arr[] = $result[1]." - ".$result[2];
		if ($result[3] > 0){
			$againstworktime_sql = "SELECT a.date, a.time, UNIX_TIMESTAMP(a.modified) as modified, a.id, lookups.name FROM againstworktime AS a JOIN lookups ON a.type = lookups.code WHERE a.member_id = {$_SESSION['SESS_MEMBER_ID']} AND a.id = {$result[3]} AND lookups.type = 'againstworktime'";
			$againstworktime_result = mysqli_query($link, $againstworktime_sql);
			if($againstworktime_result){
				
				while($againstworktime_arr = mysqli_fetch_assoc($againstworktime_result)){
					$output->arrays["againstworktime"]["item".$i] = $againstworktime_arr;	
					$i++;
				}
			}
		}
	}
}else{
	$result_arr[0] = false;
	$result_arr[] = "Nothing set";
}
$output->results['setagainstworktime'] = $result_arr;

?>