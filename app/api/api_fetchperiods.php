...
$month = clean($_REQUEST['m'] ?? null);
$year = clean($_REQUEST['y']);
$workid = clean($_REQUEST['wid'] ?? null);
$breakid = clean($_REQUEST['bid'] ?? null);
$freeid = clean($_REQUEST['fid'] ?? null);
$vacid = clean($_REQUEST['vid'] ?? null);
...
if($workid == "" && $breakid == "" && $freeid == "" && $vacid == ""){
	...
	if(mysqli_num_rows($result) == 1) {		// Check if the period was found
		$freePeriod = mysqli_fetch_object($result);
	...
	$result = mysqli_query($link, $sql); 
	if(mysqli_num_rows($result) == 1) {		// Check if the period was found
		$vacPeriod = mysqli_fetch_object($result);
...
$userinforesults = mysqli_query($link, "SELECT statsstartdate,registerdate FROM userdb WHERE member_id = ". $_SESSION['SESS_MEMBER_ID']);
