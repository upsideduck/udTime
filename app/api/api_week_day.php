...
if($periodsArray){
	foreach($periodsArray as $period){
		...
		$loopoutput['asworktime'] = $asworktimes[0]["days"][date("Y-m-d",$period[0]["starttime"])]["time"] ?? 0;
		...
		$loopoutput['againstworktime'] = $againstworktime[0]["days"][date("Y-m-d",$period[0]["starttime"])]["time"] ?? 0;
...
for($i = 1; $i <= 7; $i++){
	...
	if($againstworktime[0]["days"][$date] != null || $asworktimes[0]["days"][$date] != null){
		...
		$loopoutput['end'] = "";
		$loopoutput['break'] = "";
		if(isset($againstworktime[0]["days"][$date])) $loopoutput['againstworktime'] = $againstworktime[0]["days"][$date]['time'];
		else $loopoutput['againstworktime'] = "";
		if(isset($asworktimes[0]["days"][$date])) $loopoutput['asworktime'] = $asworktimes[0]["days"][$date]['time'];
