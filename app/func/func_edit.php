...
function updateasworktime($id, $time, $type) {
	...
	if($type == '') {
		$qry="SELECT * FROM asworktime WHERE id = $id";
		$result=mysqli_query($link, $qry);
		$vac = mysqli_fetch_object($result);
		$type = $vac->type;
	}
...
function updateagainstworktime($id, $time, $type) {
	...
	if($type == '') {
		$qry="SELECT * FROM againstworktime WHERE id = $id";
		$result=mysqli_query($link, $qry);
		$free = mysqli_fetch_object($result);
		$type = $free->type;
	}
 ...
 *		  	
 ********************************************************************/
function addPeriod($type,$newStartTime, $newEndTime, $newComment, &$id) {
	include(__SITE_BASE__."/includes/connection.php");
	$result_arr[0] = true;
	//Input Validations
	if($newStartTime == '') {
	...
	if($result_arr[0]) {
		...
		$qShowStatusResult 	= mysqli_query($link, $qShowStatus) or die ( "Query failed: " . mysqli_error() . "<br/>" . $qShowStatus );
