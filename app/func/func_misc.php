...
function fetchStartEndTime($type, $year = null, $month = null, $day = null, $week = null) {
	...
	switch ($type) {
		...
		case ("week"):
			...
			//if($year == 2015) $timeArray["start"] = $timeArray["start"]+(60*60*24*7);		// Strange bug
...
function workingdaysmonth($month,$year) {
	...
	if(is_numeric($month) && is_numeric($year)) { //need to be a numbers
		...
		$statsstartdateresult = mysqli_query($link, "SELECT statsstartdate FROM userdb WHERE member_id = ". $_SESSION['SESS_MEMBER_ID']);
		$statsstartdate = mysqli_result($statsstartdateresult,0);
 ...
 *
 ********************************************************************/ 
function lookupsTable($type) {
	include(__SITE_BASE__."/includes/connection.php");
	$lookupsresult = mysqli_query($link, "SELECT * FROM lookups WHERE type = '{$type}'");
	$lookups_arr= array();
	while ($obj = mysqli_fetch_object($lookupsresult)) {
		$lookups_arr[] = $obj;
	}
	mysqli_close($link);
	return $lookups_arr;
