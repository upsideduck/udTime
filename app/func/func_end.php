...
function endWork($comment, $timestamp) {  
	...
	$sqlSyncCheck = "SELECT * FROM workdb WHERE endtime = {$timestamp} AND  member_id = ".$_SESSION['SESS_MEMBER_ID'];
	...
	if ($_SESSION['SESS_ACTIVE_TYPE'] == "work") {
		...
		$result_req1 = mysqli_query($link, $sql_req1);
		$activeWorkInfo = mysqli_fetch_array($result_req1);
		...
		if ( $result_req2 = mysqli_query($link, $sql_req2) ) {
			while($break = mysqli_fetch_array( $result_req2 )) {
...
function endBreak($comment, $timestamp) {  
	include(__SITE_BASE__."/includes/connection.php");
	// Check if already synced, in that case, just continue with successfull result
	$sqlSyncCheck = "SELECT * FROM breakdb WHERE endtime = {$timestamp} AND  member_id = ".$_SESSION['SESS_MEMBER_ID'];
	$resultSyncCheck = mysqli_query($link, $sqlSyncCheck);
	if (mysqli_num_rows($resultSyncCheck) > 0) {
		$result_arr[0] = true;	 
	...
	if($_SESSION['SESS_ACTIVE_TYPE'] == "break"){
	    ...
	    $resultbreak = mysqli_query($link, $sql);
	    $break = mysqli_fetch_object($resultbreak); 
	    ...
	    if ($success1) {
	    	...
	    	if($success2 = mysqli_query($link, $sql2)){
