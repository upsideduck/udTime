...
function AgendaEventRenderer() {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			if (levelI) {
				// indented and thin
				//outerWidth = availWidth / (levelI + forward + 1);			JADELL
				outerWidth = availWidth;
			}else{
				if (forward) {
					// moderately wide, aligned left still		
					//outerWidth = ((availWidth / (forward + 1)) - (12/2)) * 2; // 12 is the predicted width of resizer =		JADELL
					outerWidth = availWidth;
				}else{
			...
			//left = leftmost +                                  // leftmost possible			JADELL
